...
class TokenizerLoadingConfig:
    ...
    default_pad_token = "<|finetune_right_pad_id|>"  # default to llama3's pad token, consider setting it to the right pad token if the model is not llama3
    default_eos_token = "</s>"
    default_bos_token = "<s>"
...
class ModelLoadingConfig:
    ...
    low_cpu_mem_usage: bool = True  # note: transforms DeepSpeed Zero-3 is not compatible with `low_cpu_mem_usage=True` or with passing a `device_map`, set it to False in that case
    ...
    merge_method = "dlerp"  # slerp, lerp, dlerp, dlerpin, proj or prog, used when expand_type is concat or hybrid
    ...
    interpolation_loss_type = "cosine"  # 'cosine', 'dot', 'mse', or 'attention', used when expand_type is concat or hybrid. 'attention' needs output_attentions=True
    interpolation_norm_alpha = 0  # scale to relax the layer norm, set to 0 to disable it, set to -1 to be dynamic - use alpha=interp_weight, used when expand_type is concat or hybrid
    freeze_interpolation_factor = False  # False to make it trainable, used when merge_method is slerp or lerp. For dlerp, it freezes the bias term if True
