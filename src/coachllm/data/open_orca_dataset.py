...
def get_dataset(dataset_config, tokenizer, split):
    ...
    dataset = datasets.load_dataset(dataset_config.dataset, name="default-72b2a90e74f1d4f2", split=split, cache_dir=dataset_config.hf_hub_dataset_cache_dir)
    ...
    def to_dialog(system_prompt, user_prompt, response):
        dialog = []
        dialog.append({
                "role": "system",
    ...
    dataset = dataset.map(lambda x: to_dialog(x[dataset_config.system_column], x[dataset_config.prompt_columns], x[dataset_config.response_column]), remove_columns=list(dataset.features), num_proc=dataset_config.max_workers)
