...
- predictions (array-like of shape (n_samples,) or (n_samples, n_classes)): Model predictions. Expects different inputs based on use case:
    - binary: expects an array-like of shape (n_samples,)
...
- labels (array-like of shape (n_classes,)): Only used for multiclass targets. List of labels that index the classes in
    `predictions`. If `None`, the numerical or lexicographical order of the labels in
    `predictions` is used. Defaults to `None`.
Returns:
...
Examples:
    Example 1:
        ...
        >>> results = roc_auc_score.compute(references=refs, predictions=pred_scores)
    ...
    Example 2:
        ...
        ...                 [0.1, 0.1, 0.8],
        ...                 [0.1, 0.7, 0.2]]
        >>> results = roc_auc_score.compute(references=refs, predictions=pred_scores, multi_class='ovr')
        >>> print(round(results['roc_auc'], 2))
        0.85
    ...
    Example 3:
        ...
        ...                 [0.1, 0.1, 0.8],
        ...                 [0.1, 0.7, 0.2]]
        >>> results = roc_auc_score.compute(references=refs, predictions=pred_scores, average=None)
