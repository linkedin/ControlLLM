...
def get_dataset(dataset_config, tokenizer, split):
    ...
    for name in dataset_config.names:
        subset = datasets.load_dataset(dataset_config.dataset, name=name, split=split, cache_dir=dataset_config.hf_hub_dataset_cache_dir)
    ...
    dataset = dataset.map(lambda x: to_dialog(x[dataset_config.prompt_columns], x[dataset_config.response_column]), remove_columns=list(dataset.features), num_proc=dataset_config.max_workers)
