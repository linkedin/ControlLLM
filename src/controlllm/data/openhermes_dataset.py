...
def get_dataset(dataset_config, tokenizer, split):
    # note that the dataset name is different from the one in the original code, to enable loading from cache, teknium/OpenHermes-2.5->teknium/open_hermes-2.5
    dataset = datasets.load_dataset("teknium/open_hermes-2.5", name="default", split=split, cache_dir=dataset_config.hf_hub_dataset_cache_dir)
    ...
    def to_dialog(conversations):
        ...
        for i, conversation in enumerate(conversations):
            if conversation["from"] == "system":
                ...
                raise ValueError(f"Unknown role: {conversation['from']}")
